...
from simiir.search.interfaces import Response
...
class ConversationalMemory(Memory):
    ...
    def __init__(self, search_interface, output_controller, topic):
        ...
        self._irrelevant_responses = []
        self.initial_predetermined_uttterance_amount = 0
        #self._utterance_count = 0               # Number of utterances issued in the current search session.
    ...
    def _set_response_action(self):
        ...
        Any modifications to the search context can be undertaken here.
        """
        self._responses_examined.append(self.get_last_response())
    ...
    def add_issued_utterance(self, utterance_text):
        ...
        """
        if utterance_text not in self._issued_utterances:
    ...
    def add_irrelevant_response(self, response):
        ...
        self._irrelevant_responses.append(response)


    def add_relevant_response(self, response):
        
        self._relevant_responses.append(response)

    def add_response(self, response):
        """
        Adds a response to the list of responses that have been examined.
        """
        if response not in self._responses_examined and response != "Last Response":
            self._responses_examined.append(response)
    ...
    def get_last_response(self):
        if self._last_utterance is None:
            return None
        response_content = self._search_interface.issue_utterance(self._last_utterance)
        self._last_response = Response(content = response_content, judgment=0)
        #self._last_response = self._search_interface.issue_utterance(self._last_utterance)
