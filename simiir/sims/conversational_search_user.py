...
class SimulatedConversationalUser(SimulatedBaseUser):
    ...
    #    print(utterance_text, "utterance text")
    #    if utterance_text:
    ...
    def _do_utterance(self):
        ...
        if utterance_text:
            print(f"Issued utterance: {utterance_text}")
            #if utterance_text not in self._user_context.get_issued_utterances():
            self._user_context.add_issued_utterance(utterance_text)
            self._logger.log_action(Actions.UTTERANCE, utterance=utterance_text)
            self._output_controller.log_query(utterance_text)
            return True
        else:
            print("No more utterances available.")
            self._output_controller.log_info(info_type="OUT_OF_UTTERANCES")
            self._logger.log_action(Actions.NO_MORE_UTTERANCES)
            return False
    ...
    def _do_assess_response(self):
        ...
        judgment = False
        #print(type(self._user_context.get_last_response()))
        if self._user_context.get_last_response():
            response = self._user_context.get_last_response()
            #print(self._user_context.get_all_examined_responses(), "last response")
            #print(type(response), "response type")
            
            #if response.content not in self._user_context.get_all_examined_responses():
            if response.content not in [r.content for r in self._user_context.get_all_examined_responses()] and len(response.content) > 0:
                self._user_context.add_response(response)
            self._logger.log_action(Actions.RESPONSE, status="EXAMINING_RESPONSE", response=response)

            if self._response_classifier.is_relevant(response.content):
                response.judgment = 1
